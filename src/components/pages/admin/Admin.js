...
const Admin = ({ fetchAllUser, allUser }) => {
  ...
  const allUserMap =
    Object.keys(allUser).length &&
    sortedAllUser(filterAllUser(allUser, searchField), "createdAt", "asc").map(
      (item, index) => (
        <TableRow key={item.email + index}>
          ...
          <TableCell align="right">
            <Link
              ...
              to={`/admin/editaccount/${item.userId}`}
            >
              <EditIcon />
            </Link>
          </TableCell>
